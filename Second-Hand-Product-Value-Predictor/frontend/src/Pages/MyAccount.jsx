...
function MyAccount() {
  ...
  useEffect(() => {
    ...
    const fetchCars = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/get_user/${userId}`);
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    await axios.put("http://127.0.0.1:8000/change-user-details",userData)
  ...
  const managePassword = async (e) => {
    ...
    const respond = await axios.put("http://127.0.0.1:8000/change-password",changePassword);
    console.log("hi")
